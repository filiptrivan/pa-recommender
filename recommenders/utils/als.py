...
def get_recommendation_result_dict(model: RecommenderBase, sparse_user_product, user_ids, product_names, filter_items):
    recommendations_dict = {}
    top_ten_overall_recommendations = []
    batch_size = 1000
    to_generate = np.arange(len(user_ids))
    
    for startidx in range(0, len(to_generate), batch_size):
        batch = to_generate[startidx : startidx + batch_size]
        product_indexes, scores = model.recommend(batch, sparse_user_product[batch], filter_already_liked_items=False, filter_items=filter_items)
        for i, userid in enumerate(batch):
            ...
            for product_index, score in zip(product_indexes[i], scores[i]):
                ...
                if len(top_ten_overall_recommendations) < 10:
                    top_ten_overall_recommendations.append((score, product_name))
                else:
                    min_score, min_product = min(top_ten_overall_recommendations, key=lambda x: x[0])
                    if score > min_score:
                        top_ten_overall_recommendations.remove((min_score, min_product))
                        top_ten_overall_recommendations.append((score, product_name))
            recommendations_dict[user_id] = product_ids_for_recommendation
    
    top_ten_overall_recommendations.sort(key=lambda x: x[0], reverse=True)
    recommendations_dict['top_ten_overall_recommendations'] = top_ten_overall_recommendations
